...
\end{figure}
\begin{itemize}
\only<1>{\item Every latent dimensions are heavily penalized with $\beta_h$.}
\onslide<2->{\item Each penalty on latent dimension is sequentially relieved one at a time with $\beta_l$ in a cascading fashion.}
\end{itemize}
\end{frame} 
...
\item We evaluate with disentanglement score introduced in \textbf{FactorVAE} \footnote{{\color{blue}{Kim, H. and Mnih, A.}} Disentangling by factorising. {\color{gray}{ICML2018}}} and unsupervised classification accuracy.
