...
\begin{itemize}\itemsep=12pt
\item Our interest is to disentangle the underlying explanatory factors of data without any supervision.\pause
\item We require a stable disentanglement framework to capture continuous and discrete factors.\pause
\item VAE framework guarantees stable framework compared to GAN framework.
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep=12pt
\item We propose a simple procedure for penalizing the total correlation without any extra discriminator network \footnote{{\color{blue}{Kim, H. and Mnih, A.}} Disentangling by factorising. {\color{gray}{ICML2018}}} or importance sampling \footnote{{\color{blue}{Chen, T. Q., Li, X., Grosse, R. B., and Duvenaud, D. K.}} Isolating sources of disentanglement in variational autoencoders. {\color{gray}{NIPS2018}}} in the $\beta$-VAE \footnote{{\color{blue}{Higgins, I., Matthey, L., Pal, A., Burgess, C., Glorot, X., Botvinick, M., Mohamed, S., and Lerchner, A.}} beta-vae: Learning basic visual concepts with a constrained variational framework. {\color{gray}{ICLR2017}}} framework.\pause
\item We propose an alternating disentanglement method to disentangle discrete and continuous factors without lumping both the continuous and discrete factors into a single latent vector.
\end{itemize}
\end{frame}
...
\item Penalizing TC leads the model to learn statistically independent factors in the data which is a crucial component in disentangled representations. 
%%\begin{align}
...
\begin{itemize}\itemsep=12pt
\item \[TC(z) = \sum_{i=2}^m I(z_{1:i-1}; z_i)\]
\footnote{
\begin{align}
...
\only<5>{\includegraphics[page=5, width=\linewidth]{arch}}
\end{figure}
\begin{itemize}
\item In practice, each latent dimensions are heavily penalized with high $\beta$ in the beginning but are sequentially relieved one at a time with small $\beta$ in a cascading fashion. 
\end{itemize}
\end{frame} 
...
%%\begin{itemize}
%%\item 
%% We define the joint distribution of the variables as 
%%\begin{align}
%%    q_{\phi,\theta}(x,z,d)=\begin{cases} q_\phi(x,z) & \text{if}~d=\argmax_{\hat{d}} p_\theta(x \mid z,\hat{d})\\ 0 & \text{otherwise} \end{cases} \nonumber,
%%\end{align}
%%where $q_\phi(x,z)=p(x)q_\phi(z\mid x)$.
%%\item 
%%Likewise, the variational posterior is defined as,
%%\begin{align}
%%    q_{\theta,\phi}(z,d\mid x)=\begin{cases} q_\phi(z\mid x) & \text{if}~d=\argmax_{\hat{d}} p_\theta(x\mid z,\hat{d})\\ 0 & \text{otherwise} \end{cases}\nonumber
%%\end{align}
%%\end{itemize}
%%\end{frame} 
...
\begin{itemize}\itemsep=12pt
\item Disentanglement score is obtained from 10 different random seed each with the best hyperparameters. 
...
\begin{column}{0.4\textwidth}
\begin{itemize}\itemsep=12pt
\item Unsupervised classification results when size of discrete variable is 3.
...
\begin{column}{0.4\textwidth}
\begin{itemize}\itemsep=12pt
\item Unsupervised classification results  when size of discrete variable is 10.
